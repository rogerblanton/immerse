...
%>
    <div class="form_row">
      ...
      <div class="form_rightcol form_rightcolnooverflow">
        ...
        if ( src.length() > 0 ) {
            out.write(" src=\"");
            out.write(xssAPI.getValidHref(Text.escapePath(slingRequest.getContextPath() + src)));
            out.write("\"");
        }
        if ( title.length() > 0 ) {
            out.write(" alt=\"");
        ...
        if ( FormsHelper.doClientValidation(slingRequest) ) {
            ...
            out.write(xssAPI.encodeForHTMLAttr(FormsHelper.getFormsPreCheckMethodName(slingRequest)));
            out.write("('");
            if ( name.length() > 0 ) {
                out.write(xssAPI.encodeForJSString(name));
            }
            out.write("')) { document.forms['");
            out.write(xssAPI.encodeForJSString(FormsHelper.getFormId(slingRequest)));
