...
%><%@include file="/libs/foundation/global.jsp"%><%
    ...
    boolean inheritanceCancelled = properties.get("inheritance", "").equals("cancel");
    for (Paragraph par: parSys.paragraphs()) {
        Resource r = (Resource) par;
        // if there is a fake inheritance resource and if child inheritance is cancelled, we draw the resource
        if (r.getResourceType().endsWith("/iparsys/par")) {
            hasFake = true;
            if (inheritanceCancelled) {
            ...
            switch (par.getType()) {
                case START:
                    ...
                    // open outer div
                    %><div class="parsys_column <%= xssAPI.encodeForHTMLAttr(par.getBaseCssClass()) %>"><%
                    // open column div
                    %><div class="parsys_column <%= xssAPI.encodeForHTMLAttr(par.getCssClass()) %>"><%
                    hasColumns = true;
                    break;
                case BREAK:
                    if (editContext != null) {
                    ...
                    %></div><div class="parsys_column <%= xssAPI.encodeForHTMLAttr(par.getCssClass()) %>"><%
                    break;
                case END:
                    if (editContext != null) {
                        // draw new bar
    ...
    // include fake inheritance if not present in the content or if child inheritance is not cancelled
    if (!hasFake || !inheritanceCancelled) {
