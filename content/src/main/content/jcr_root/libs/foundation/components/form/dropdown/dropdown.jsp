...
%><%@ page import="java.util.LinkedHashMap,
    ...
    final String cssClass = xssAPI.encodeForHTMLAttr(FormsHelper.getCss(properties, "form_field form_field_select"));
%>
    <div class="form_row">
       ...
       <div class="form_rightcol">
        <%
        if ( multiSelect ) {
            %><select class="<%=cssClass%>" id="<%=id %>" name="<%=name%>" multiple="multiple" <%= (width.length()>0?"style='width:"+width+"px;'":"") %>><%
        } else {
            %><select class="<%=cssClass%>" id="<%=id %>" name="<%=name%>" <%= (width.length()>0?"style='width:"+ width +"px;'":"") %>><%
        }
