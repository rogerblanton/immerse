...
<%@include file="/libs/foundation/global.jsp"%><%
    ...
    %><li><p><a href="<%= xssAPI.getValidHref(listItem.getPath()) %>.html" title="<%= StringEscapeUtils.escapeHtml4(title) %>"
                onclick="CQ_Analytics.record({event: 'listNewsItemClicked', values: { listItemPath: '<%= xssAPI.getValidHref(listItem.getPath()) %>' }, collect:  false, options: { obj: this }, componentPath: '<%=xssAPI.getValidHref(resource.getResourceType())%>'})"><%
