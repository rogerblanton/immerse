...
%><%@include file="/libs/foundation/global.jsp"%><%
    ...
    %><li<%=request.getAttribute("listitemclass")%>><p><a href="<%= xssAPI.getValidHref(listItem.getPath()) %>.html" title="<%= StringEscapeUtils.escapeHtml4(title) %>"><%

    if (hasImage && MobileUtil.hasCapability(slingRequest, DeviceCapability.CAPABILITY_IMAGES)) {
        %><img class="teaser" src="<%= xssAPI.getValidHref(listItem.getPath()) %>.thumbnail.jpg" alt="<%= StringEscapeUtils.escapeHtml4(title) %>" title="<%= StringEscapeUtils.escapeHtml4(title) %>"><%
    }
